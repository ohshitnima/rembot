...
public class MessageListenerExample extends ListenerAdapter {
	...
	public static void main(String[] args) {
		...
		try {
			JDA jda = new JDABuilder("NDk1NTQ0MDU1MzgwNDQzMTQ2.DpI-zQ.XuR1FDzM9W9Lf6vfcAwj938Uxy0") // The token of the
																									// account that is
																									// logging in.
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		// Now that you have a grasp on the things that you might see in an event,
		// specifically MessageReceivedEvent,
		...
		// If you did message.equals() it will fail because you would be comparing a
		// Message to a String!
		if (msg.equals("!ping")) {
			// This will send a message, "pong!", by constructing a RestAction and
			// "queueing" the action with the Requester.
			// By calling queue(), we send the Request to the Requester which will send it
			// to discord. Using queue() or any
			// of its different forms will handle ratelimiting for you automatically!

			channel.sendMessage("pong!").queue();
		} else if (msg.equals("!roll")) {
			// In this case, we have an example showing how to use the Success consumer for
			// a RestAction. The Success consumer
			...
			// success consumer to determine when
		...
		{
			// This is an admin command. That means that it requires specific permissions to
			// use it, in this case
			// it needs Permission.KICK_MEMBERS. We will have a check before we attempt to
			// kick members to see
			// if the logged in account actually has the permission, but considering
			// something could change after our
			// check we should also take into account the possibility that we don't have
			// permission anymore, thus Discord
			...
			if (message.isFromType(ChannelType.TEXT)) {
				...
				if (message.getMentionedUsers().isEmpty()) {
					...
					Member selfMember = guild.getSelfMember(); // This is the currently logged in account's Member
																// object.
					...
					for (User user : mentionedUsers) {
						Member member = guild.getMember(user); // We get the member object for each mentioned user to
																// kick them!
						...
						if (!selfMember.canInteract(member)) {
							// use the MessageAction to construct the content in StringBuilder syntax using
							// append calls
						...
						guild.getController().kick(member).queue(success -> channel.sendMessage("Kicked ")
								.append(member.getEffectiveName()).append("! Cya!").queue(), error -> {
									...
									if (error instanceof PermissionException) {
										...
										Permission missingPermission = pe.getPermission(); // If you want to know
																							// exactly what permission
																							// is missing, this is how.
																							// Note: some
			...
			// This is an example of how to use the complete() method on RestAction. The
			// complete method acts similarly to how
			// JDABuilder's awaitReady() works, it waits until the request has been sent
			// before continuing execution.
			// Most developers probably wont need this and can just use queue. If you use
			// complete, JDA will still handle ratelimit
			// control, however if shouldQueue is false it won't queue the Request to be
			// sent after the ratelimit retry after time is past. It
			// will instead fire a RateLimitException!
			// One of the major advantages of complete() is that it returns the object that
			// queue's success consumer would have,
			// but it does it in the same execution context as when the request was made.
			// This may be important for most developers,
			// but, honestly, queue is most likely what developers will want to use as it is
			// faster.
			...
			try {
				...
				// the message was sent
				// It does handle rate limits automatically
				Message sentMessage = channel.sendMessage("hello wswm").complete();
